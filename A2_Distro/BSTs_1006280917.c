...
BST_Node *newBST_Node(double freq, int bar, double index)
{
	...
	BST_Node *node_ptr = (BST_Node *)calloc(1, sizeof(BST_Node));
	if(node_ptr != NULL && bar >= 0 && index >= 0.0 && index <= 1.0)
	{
		node_ptr -> freq = freq;
		node_ptr -> bar = bar;
		node_ptr -> index = index;
		node_ptr -> key = (10.0*(double)bar)+index;
		node_ptr -> left = NULL;
		node_ptr -> right = NULL;
...
BST_Node *BST_search(BST_Node *root, int bar, double index)
{
    ...
    ****/
	double calc_key = (10.0*bar)+index;
	...
	{
		/* //printf("RK = %f ---- CK = %f\n", root -> key, calc_key);
		//BST_Node *find = BST_search(root -> left, bar, index);
		//printf("%f\n", find -> key); */
		return BST_search(root -> left, bar, index);
	}
	else if(root -> key < calc_key)
	{
...
BST_Node *find_successor(BST_Node *right_child_node)
{
    ...
    ****/
	...
	BST_Node *temp = right_child_node;
...
BST_Node *BST_delete(BST_Node *root, int bar, double index)
{
    ...
    ****/
	BST_Node *temp = NULL;
	double calc_key = (10.0*(double)bar)+index;
...
void BST_replace(BST_Node *root, double in_freq, double out_freq)
{
	if(root != NULL)
	{
		if(root -> freq == in_freq)
		{
			root -> freq = out_freq;
		}
		BST_replace(root -> left, in_freq, out_freq);
		BST_replace(root -> right, in_freq, out_freq);
	}
}
...
void BST_shiftFreq(BST_Node *root, char note_src[5], char note_dst[5])
{
	...
	//relate note_src to its freq 
	int i = 0;
	int found = 0;
	double note_src_freq;
	
	while(i < 100 && found != 1)
	{
		if (strcmp(note_names[i], note_src) == 0)
		{
			note_src_freq = note_freq[i];
			found = 1;
		}
		i++;
	}
	//relate note_dst to its freq 
	i = 0;
	int found1 = 0;
	double note_dst_freq;
	
	while(i < 100 && found1 != 1)
	{
		if (strcmp(note_names[i], note_dst) == 0)
		{
			note_dst_freq = note_freq[i];
			found1 = 1;
		}
		i++;
	}
	...
	if(found == 1 && found1 == 1)
	{
		BST_replace(root, note_src_freq, note_dst_freq);
	}
}
 ...
 * ******************************************************************************************/
BST_Node *BST_harmonize_temp_tree(BST_Node *root_org, BST_Node *root_temp, int semitones, double time_shift)
{
	...
	if(root_org != NULL)
	{
		...
		int i = 0;
		int found = 0;
		BST_Node *ser_dup = BST_search(root_org, root_org -> bar, (root_org -> index) + time_shift);
		
		
		while(i < 100 && found != 1)
		{
			if(i + semitones >= 0 && i + semitones <= 99 && note_freq[i] == root_org -> freq)
			{
		...
		if(found == 1 && ser_dup == NULL)
		{
			printf("1");
			new_note = newBST_Node(new_freq, root_org -> bar, (root_org -> index) + time_shift);
			root_temp = BST_insert(root_temp, new_note);
		}
		else if (found == 1 && ser_dup != NULL && (root_org -> index) + time_shift + 0.000001 < 1 && (root_org -> index) + time_shift + 0.000001 >= 0)
		{
			printf("2");
			new_note = newBST_Node(new_freq, root_org -> bar, (root_org -> index) + time_shift + 0.000001);
			root_temp = BST_insert(root_temp, new_note);
		}
		else if (found == 1 && ser_dup != NULL && (root_org -> index) + time_shift - 0.000001 < 1 && (root_org -> index) + time_shift - 0.000001 >= 0)
		{
		...
		BST_harmonize_temp_tree(root_org -> right, root_temp, semitones, time_shift);
		BST_harmonize_temp_tree(root_org -> left, root_temp, semitones, time_shift);
		
	}
	return root_temp;
}

BST_Node *BST_combine_tree(BST_Node *root, BST_Node *root_temp)
{
	if(root_temp != NULL)
	{
		BST_Node *new_note=NULL;
		new_note = newBST_Node(root_temp -> freq, root_temp -> bar, root_temp -> index);
		root = BST_insert(root, new_note);
		
		root = BST_combine_tree(root, root_temp -> left);
		root = BST_combine_tree(root, root_temp -> right);
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
	...
	BST_Node *root_temp = NULL;
	root_temp = BST_harmonize_temp_tree(root, root_temp, semitones, time_shift);	
	
	root = BST_combine_tree(root, root_temp);
	
	delete_BST(root_temp);
